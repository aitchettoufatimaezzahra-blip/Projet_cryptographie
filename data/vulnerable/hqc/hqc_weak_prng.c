#include <stdlib.h>  /* VULN: added for rand() */
#include "code.h"
#include "gf2x.h"
#include "hqc.h"
...
void PQCLEAN_HQC192_CLEAN_hqc_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    rand(  /* VULN: weak PRNG */ sk_seed, SEED_BYTES);
    rand(  /* VULN: weak PRNG */ sigma, VEC_K_SIZE_BYTES);
    PQCLEAN_HQC192_CLEAN_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    rand(  /* VULN: weak PRNG */ pk_seed, SEED_BYTES);
